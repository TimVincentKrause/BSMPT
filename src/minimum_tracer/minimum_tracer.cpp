    ...
    const int &maxiter)
{
  ...
  if (L2NormVector(df(guess)) < error or abs(df(guess).at(1)) < error) return guess; //EU!!!
  ...
  for (i = 0; (i < maxiter) && (L2NormVector(df(new_guess)) > error or abs(df(new_guess).at(1)) > error); i++) //EU!!!
  {
  ...
  while ((finalT - currentT) / dT >= 0)
  {
    ...
    if (abs(dT) < 1e-5)
    {
      break;
             ...
             abs(dV(point).at(1)) > GradientThreshold or //EU!!! // this is for the ew vev tracking to T=0; 
                         ...
                         [](double v) { return isnan(v); }))
    {
      ...
      ss << "\n\033[1;95m.-> | T = " << currentT
         << " |Grad|/dim =  " << LengthGradient 
         << " |GradSM| = " << abs(dV(new_point).at(1)) //EU!!!
         << " | Distance = " << Distance
                          ...
                          const bool &unprotected)
{
  ...
  while ((finalT - currentT) / dT >= 0)
  {
    ...
    LengthGradient =
        L2NormVector(dV(new_point)) / dim; // (1 + currentT * currentT) *
    // If BSM direction gets stuck -> only look at SM part: (CPVDM) //EU!!!
    double LengthGradientwsm = 1000000; //EU!!!
    if (new_point.size() > 1 ) // this is to also use the SM for example //EU!!!
    {
      LengthGradientwsm = abs(dV(new_point).at(pos_smvev)); //EU!!!
    }
    // Compare minimum and previous iteration
    Distance = L2NormVector(new_point - point);
    // Compute difference in energy between both minimum
    ...
    else if (LengthGradient > GradientThreshold or //  GradientThreshold = 1e-3 normally
             LengthGradientwsm > GradientThreshold or //EU!!!
             Distance > ThresholdDistance or
             std::any_of(new_point.begin(),
                         new_point.end(),
                         [](double v) { return isnan(v); }))
