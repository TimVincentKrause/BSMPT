                                     ...
                                     int diff) const
{
  ...
  else if (diff == -1) //Carlo: With the new function "bosonTderiv" below, this piece, that contains an error which is corrected below, should not be used anymore
  {
    res += 1.0 / (2 * std::pow(M_PI, 2)) *
           (4*std::pow(Temp, 3) * ThermalFunctions::JbosonNumericalIntegration(Ratio, 0) -
            (2*Temp*MassSquared) * ThermalFunctions::JbosonNumericalIntegration(Ratio, 1)); //there is a piece missing here inside the bracket
  }
  return res;
}

//Carlo: defining a new function to perform the derivative with respect to temperature
                                     ...
                                     int diff) const
{
  ...
  { 
    ...
    res += 1.0 / (2 * std::pow(M_PI, 2)) * (4 * std::pow(Temp, 3) * m_thermalTables.JB(Ratio) -
            (2 * Temp * MassSquared - Temp*Temp * DevMassSquared) * m_thermalTables.DJB(Ratio));
  }
  return res;
}
...
Class_Potential_Origin::fermion(double MassSquared, double Temp, int diff) const
{
  ...
  else if (diff == -1)
  {
    ...
    res += 1.0 / (2 * std::pow(M_PI, 2)) * (4 * std::pow(Temp, 3) * m_thermalTables.JF(Ratio) -
            2 * Temp * MassSquared * m_thermalTables.DJF(Ratio));
  }
  return res;
}
                                           ...
                                           const int &diff) const
{
  ...
  auto MassMatrix = HiggsMassMatrix(v, Temp, 0); //Carlo correction, old implementation above 
                                      ...
                                      int diff) const
{
  ...
  //Carlo commments here
  //V1loop in the Parwani implementation is modified for the time being to match the potential for the wall velocity; 
  //In particular, contributions from light fermions are neglected. 
  //Contributions from scalars are put to 0 in derivative terms as there are issues in the definition of the vector HiggsMassSquared in the derivatives of the potential
  ...
  if (diff == 0)
  {
    ...
    { 
      for (std::size_t k = 0; k < NHiggs; k++){
        ...
        //std::cout << " --- debug pot scalars : " << HiggsMassesVec[k] << " " <<  xx << std::endl; //Carlo debug 
        }
      for (std::size_t k = 0; k < NGauge; k++) {
        res += boson(GaugeMassesVec[k], Temp, C_CWcbGB, 0);
        //std::cout << " --- debug pot GB long : " << GaugeMassesVec[k] << " " << xx << std::endl; //Carlo debug
        }
      for (std::size_t k = 0; k < NGauge; k++){
        res += 2 * boson(GaugeMassesZeroTempVec[k], Temp, C_CWcbGB, 0);
        //std::cout << " --- debug pot GB transv : " << GaugeMassesZeroTempVec[k] << " " << xx << std::endl;   //Carlo debug
      }
      for (std::size_t k = 0; k < NQuarks; k++) { 
        //std::cout << " --- debug pot quarks : " << QuarkMassesVec[k] << " " <<  -6 * fermion(QuarkMassesVec[k], Temp, 0) << std::endl; //Carlo debug
        res += -6 * fermion(QuarkMassesVec[k], Temp, 0);
      }
      for (std::size_t k = 0; k < NLepton; k++) {
        //std::cout << " --- debug pot leptons : " << LeptonMassesVec[k] << " " <<  -2 * fermion(LeptonMassesVec[k], Temp, 0) << std::endl; //Carlo debug
  ...
  else if (diff > 0 and static_cast<size_t>(diff) <= NHiggs)
  {
    ...
    {  
      ...
      { 
        ...
        //std::cout << " --- debug : " << C_CWcbHiggs << " " << HiggsMassesVec.at(k) << " " << HiggsMassesVec.at(k + NHiggs) << " " << HiggsMassesVec.at(k + NHiggs) * boson(HiggsMassesVec.at(k), Temp, C_CWcbHiggs, diff) << std::endl; //Carlo debug
      }
      
      
      for (std::size_t k = 0; k < NGauge; k++)
      {
        res += GaugeMassesVec.at(k + NGauge) * boson(GaugeMassesVec.at(k), Temp, C_CWcbGB, diff); //Carlo: correcting the coefficient C_CWcbHiggs -> C_CWcbGB
        //std::cout << " --- debug : " << C_CWcbGB << " " << GaugeMassesVec.at(k) << " " << GaugeMassesVec.at(k + NGauge) << " " << GaugeMassesVec.at(k + NGauge) * boson(GaugeMassesVec.at(k), Temp, C_CWcbHiggs, diff) << std::endl; //Carlo debug
      }
      
      for (std::size_t k = 0; k < NGauge; k++)
      {
        res += 2 * GaugeMassesZeroTempVec.at(k + NGauge) *
               boson(GaugeMassesZeroTempVec.at(k), Temp, C_CWcbGB, diff); // Carlo: correcting the coefficient C_CWcbHiggs -> C_CWcbGB
        //std::cout << " --- debug : " << C_CWcbGB  << " " << GaugeMassesZeroTempVec.at(k) << " " << GaugeMassesZeroTempVec.at(k + NGauge) << " " << C_CWcbHiggs << " "  << 2 * GaugeMassesZeroTempVec.at(k + NGauge) * boson(GaugeMassesZeroTempVec.at(k), Temp, C_CWcbHiggs, diff) << std::endl; //Carlo debug
      }
      ...
      for (std::size_t k = 0; k < NQuarks; k++) 
      {
        ...
        //std::cout << " --- debug : "<< 1.5 << " " << QuarkMassesVec[k] << " " << QuarkMassesVec.at(k + NQuarks) << " " << -6 * QuarkMassesVec.at(k + NQuarks) * fermion(QuarkMassesVec[k], Temp, diff) << std::endl; //Carlo debug
      }
