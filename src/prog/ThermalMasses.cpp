...
try
{
  ...
  while (getline(infile, linestr))
  {
    ...
    if (linecounter == args.line)
    {
      ...
      // // Hessian Tree diagonals
      // for (std::size_t j = 0; j < modelPointer->get_NHiggs(); j++){
      //   for (std::size_t k = 0; k < modelPointer->get_NHiggs(); k++)
      //     LegendMinima.push_back("RotHess_" + to_string(j)+ to_string(k));
      // }
      // for (std::size_t j = 0; j < modelPointer->get_NHiggs(); j++)
      //   LegendMinima.push_back("mSpot_" + to_string(j) + "sq_T");
      ...
      for (const auto T : T_list)
      {
        ...
        /*
          ...
          maybe need to check correct parametrisation
          if R11 < 0 -> flip h1->-h1
          if R33 < 0 -> flip h3-> h3
          if det R < 0 -> h2->-h2
          w/ detR = -R02*R11* R20+R01* R12* R20+R02* R10* R21-R00* R12* R21-R01* R10* R22+R00* R11* R22



         // somewhere output maybe vac.CoexPhasesList.at(0).crit_temp for all critical temperatures
        ...
        */
        ...
        //std::vector<double> err(vev.size());
        ...
        //err = HessianNumericalhighaccerr(modelPointer->MinimizeOrderVEV(vev),Veff, eps);

        outfile << sep
                << Hessian;

        MatrixXd eigenhess(modelPointer->get_NHiggs(),modelPointer->get_NHiggs());
        //eigenhess = Hessian;//Hessian(modelPointer->get_NHiggs(),modelPointer->get_NHiggs());
        ...
        //es.compute(eigenhess, EigenvaluesOnly);
        //auto HiggsRot = es.eigenvectors().transpose();
        ...
        // find out which row / columns the mHp is
        //std::cout << MassSquaredHiggs.size() << std::endl;
        // first load tmpmass as first possible mHp^2
        double tmpmass = MassSquaredHiggs[5];
        size_t i_mHp = 6;
        // go through every possible mHp^2 and break if duplicate
        while (i_mHp<8){
          //std::cout << i << std::endl;
        ...
        //std::function<double(std::vector<double>)> Rrot();

        // c1 = R00/ (np.sqrt(1-R02**2))
        // s1 = R01/ (np.sqrt(1-R02**2))

        // c2 = np.sqrt(1-R02**2)
        // s2 = R02

        // c3 = R22 / (np.sqrt(1-R02**2))
        // s3 = R12 / (np.sqrt(1-R02**2))

        /*
        TRANSPOSED:
        R00: HiggsRot(6,4)  R10: HiggsRot(6,5)  R10: HiggsRot(6,8)
        R01: HiggsRot(7,4)  R10: HiggsRot(7,5)  R10: HiggsRot(7,8)
        R02: HiggsRot(8,4)  R10: HiggsRot(8,5)  R10: HiggsRot(8,8)

        
        ->
        */

        // std::cout << "R01 =" << HiggsRot(4,7) << std::endl;
        // std::cout << "R02 =" << HiggsRot(4,8) << std::endl;
        // std::cout << "R12 =" << HiggsRot(5,8)  << std::endl;
        ...
        // std::cout << "a1 =" << std::asin(s1) << std::endl;
        // std::cout << "a2 =" << std::asin(s2) << std::endl;
        // std::cout << "a3 =" << std::asin(s3) << std::endl;
        ...
        // std::cout << "MassSquaredHiggs" << std::endl;
        // std::cout << MassSquaredHiggs << std::endl;
        // outfile << sep
        //         << Hessian;
        ...
        // std::function<double(std::vector<double>)> Vtree;
        // // Define effective potential
        // Vtree = [&](std::vector<double> effvev)
        // {
        //   // Potential wrapper
        //   return modelPointer->VTree(effvev, T);
        // };

        // //std::cout<< "T=" << T << " : " <<modelPointer->MinimizeOrderVEV(vev) << "\n";
        // Hessian = HessianNumerical(modelPointer->MinimizeOrderVEV(vev),Vtree, eps);
        // //err = HessianNumericalhighaccerr(modelPointer->MinimizeOrderVEV(vev),Veff, eps);
